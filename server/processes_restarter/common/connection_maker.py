...
class ConnectionMaker(threading.Thread):
    def __init__(
        self, create_connections_q, connected_processes_q, restart_containers_q
    ):
        super().__init__()
    ...
    def __execute_create_connections_operations(self):
        while self.active:
            ...
            if not container_name:
                if self.active:
                    logging.error(
                        f"action: connection_maker_error | error: container_is_none"
                    )
                continue
        ...
        while self.active:
            try:
                ...
                if sleep_time >= MAX_TIME_SLEEP:
                    logging.error(
                        f"action: unable_to_connect | container: {container_addr}"
                    )
