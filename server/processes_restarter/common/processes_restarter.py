...
class ProcessesRestarter:
    def __init__(
        ...
        restarters_names = [
            container_restarter_name + str(restarter_id)
    ...
    def __init_restarter(self):
        ...
        self.docker_restarter = DockerRestarter(
            self.restart_containers_q, self.create_connections_q
        )

        self.connection_maker = ConnectionMaker(
            self.create_connections_q,
            self.connected_processes_q,
            self.restart_containers_q,
        )

        self.healthy_checker = HealthyChecker(
            self.connected_processes_q, self.restart_containers_q
        )
    ...
    def __wait_while_leader(self):
        ...
        if current_state == KEEP_ALIVE_ACCEPTED:
            ...
            logging.error(
                "action: processes_restarter_error"
